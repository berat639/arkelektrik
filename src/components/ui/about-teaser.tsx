            ...
            <h2 className="font-heading font-bold text-4xl lg:text-5xl uppercase leading-tight text-gray-900 mb-4 whitespace-pre-wrap">
              {subtitle.includes(" ") ? (
                <>
                  {subtitle.split(" ")[0]}
                  <br />
                  <span className="text-ark-red-light">{subtitle.substring(subtitle.indexOf(" ") + 1)}</span>
                </>
              ) : (
                subtitle
              )}
              ...
              {HARDCODED_HIGHLIGHTS.map(({ icon, title, desc }) => {
                ...
                return (
                  <div key={title} className="flex gap-4 group">
                    <div className="flex-shrink-0 w-10 h-10 bg-ark-red/10 border border-ark-red/20 flex items-center justify-center transition-colors duration-300 group-hover:bg-ark-red/20">
                      <Icon size={18} className="text-ark-red-light" />
                    </div>
                    <div>
