...
export function AboutSections({ settings, aboutContent, aboutImage }: AboutSectionsProps) {
  ...
  return (
    <>
      ...
      <section className="bg-slate-50 py-24">
        <div className="max-w-7xl mx-auto px-6">
          <div className="grid lg:grid-cols-2 gap-16 items-start">
            ...
            >
              <span className="text-ark-red-light text-xs font-semibold uppercase tracking-[0.25em] mb-4 block">Hikayemiz</span>
              <h2 className="font-heading font-bold text-4xl uppercase text-gray-900 mb-4 whitespace-pre-wrap">
                {settings.aboutStoryTitle ? (
                  settings.aboutStoryTitle.includes(" ") ? (
                    <>
                      {settings.aboutStoryTitle.split(" ")[0]} <span className="text-ark-red-light">{settings.aboutStoryTitle.substring(settings.aboutStoryTitle.indexOf(" ") + 1)}</span>
                    </>
                  ) : (
                    settings.aboutStoryTitle
                  )
                  ...
                  <>
                    Deneyimden Doğan <span className="text-ark-red-light">Uzmanlık</span>
                  </>
                )}
              </h2>
              ...
              <ul className="space-y-3">
                {settings.expertiseItems.map((item) => (
                  <li key={item} className="flex items-start gap-3">
                    <CheckCircle size={16} className="text-ark-red-light flex-shrink-0 mt-0.5" />
              ...
              <div className="flex gap-4 mt-8 flex-wrap">
                {settings.certPartners.map(({ name, desc, image_url }) => (
                  ...
                  >
                    ...
                    <div className="relative z-10 text-ark-red-light text-xs uppercase tracking-wide mt-0.5">{desc}</div>
                  </div>
                ))}
              </div>
            </div>
      ...
      {/* CTA */}
      <section className="bg-gradient-to-br from-ark-red-muted to-ark-red-muted/40 py-20 border-t border-ark-red/15">
